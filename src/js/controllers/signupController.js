...
var signupController = function($scope, $rootScope, userServices, SharedState, errorServices, toastServices, localStorageService, config) {
	$scope.input = {
		telephone: "",
		tel_code: "",
		password: "",
	...
	$scope.next = function() {
		toastServices.show();
		userServices.checkBySignup($scope.input).then(function(data) {
			toastServices.hide()
	...
	$scope.submit = function() {
		toastServices.show();
		userServices.signup($scope.input).then(function(data) {
			toastServices.hide()
			if (data.status == config.response.SUCCESS) {
